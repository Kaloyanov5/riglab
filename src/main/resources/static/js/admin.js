// ===== RigLab Admin Dashboard (admin.html) =====
const API = '/api';
...
async function loadComponents() {
    ...
    tbody.innerHTML = '<tr><td colspan="7" class="admin-loading"><div class="spinner"></div> Loading...</td></tr>';
    ...
    try {
        ...
        const total = (data.page || data).totalElements || 0;
        document.getElementById('admin-count').textContent = `${total} component(s)`;
    } catch (err) {
        tbody.innerHTML = `<tr><td colspan="7" class="admin-loading">Error: ${err.message}</td></tr>`;
    }
}
...
function renderTable(components) {
    ...
    if (!components.length) {
        tbody.innerHTML = '<tr><td colspan="7" class="admin-loading">No components found.</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = components.map(c => `
        <tr>
            ...
            <td>${esc(c.brand)}</td>
            <td><span class="card-type" data-type="${c.type}">${c.type}</span></td>
